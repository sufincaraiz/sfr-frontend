...
export function AsesorForm() {
    ...
    try {
      const res = await fetch('/api/leads', {
        ...
        body: JSON.stringify({
          ...
          mensaje:  form.mensaje || 'Solicita asesoría de inversión (Guía de Inversión)',
          channel:  'guia-inversion',
        }),
      });
      if (!res.ok) {
  ...
  if (status === 'success') {
    return (
      <div style={{ textAlign: 'center', padding: '2rem 1.5rem' }}>
        <CheckCircle2 size={52} color="#15803D" style={{ margin: '0 auto 0.9rem' }} />
        <h3 style={{ color: '#0D2D5E', fontWeight: 800, fontSize: '1.25rem', marginBottom: 8 }}>
          ¡Recibido!
        </h3>
        <p style={{ color: '#475569', fontSize: '0.95rem', lineHeight: 1.6, maxWidth: 360, margin: '0 auto' }}>
          Un asesor experto te contactará en menos de 24 horas para orientar tu inversión.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        <label style={labelStyle} htmlFor="g-nombre">Nombre completo *</label>
        <input id="g-nombre" type="text" value={form.nombre} onChange={update('nombre')}
          placeholder="Tu nombre completo" required autoComplete="name" style={inputStyle} />
      </div>
      <div>
        <label style={labelStyle} htmlFor="g-telefono">Celular / WhatsApp *</label>
        <input id="g-telefono" type="tel" value={form.telefono} onChange={update('telefono')}
          placeholder="321 000 0000" required autoComplete="tel" style={inputStyle} />
      </div>
      <div>
        <label style={labelStyle} htmlFor="g-mensaje">¿Qué tipo de inversión te interesa?</label>
        <textarea id="g-mensaje" value={form.mensaje} onChange={update('mensaje')}
          placeholder="Lotes, fincas de descanso, apartamentos, presupuesto, zona de interés…"
      ...
      <button type="submit" disabled={status === 'loading'}
        ...
        <Send size={17} /> {status === 'loading' ? 'Enviando…' : 'Habla con un asesor experto'}
      </button>
